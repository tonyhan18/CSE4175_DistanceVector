	...
	for (int src = 0; src < g_size; ++src)
	{
		for (vector<int>::iterator dest = g_map[src].begin(); dest < g_map[src].end(); ++dest)
		{
			...
			for (d_it = g_table[*dest]->begin(), s_it = g_table[src]->begin(); d_it != g_table[*dest]->end(); d_it++, s_it++)
			{
				...
				else if (s_it->second.nexthop == -999 || (s_it->second.cost > s_to_d_cost + d_it->second.cost && g_origin[src][*dest] == s_to_d_cost))
				{
				...
				// dest 테이블 바꾸기
				else if (d_it->second.nexthop == -999 || (d_it->second.cost > d_to_s_cost + s_it->second.cost && g_origin[*dest][src] == d_to_s_cost))
				{
